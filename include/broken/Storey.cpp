...

	{
		...
		if (rooms_number % 2 == 1 || 6 * xAxis>10 * zAxis || 6 * zAxis >= 10 * xAxis) {
			...
			else
			{
				...
				double l_stairs_area.xAxis = xAxis;
	...
	void draw() {
		...
		if (rooms_number % 2 == 1 || 6 * xAxis>10 * zAxis || 6 * zAxis >= 10 * xAxis) {
			if (xAxis > zAxis)
			{
				...
				glColor4f(1, 1, 1, 0.6);
				draw_quad({ { start.x, start.y, start.z },{ start.x + xAxis - 3 * (block - roomlength) / 4, start.y, start.z },{ start.x + xAxis - 3 * (block - roomlength) / 4, start.y, start.z - zAxis } ,{ start.x, start.y, start.z - zAxis } }, { xAxis,zAxis }, out_texture, out_texture, 1, 0);
				glColor4f(1, 1, 1, 1);
				glDisable(GL_BLEND);
				// up
				glEnable(GL_BLEND);
				glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
				//	 glColor4f(0, 0, 0, 1);
				draw_quad({ { start.x, start.y + yAxis, start.z },{ start.x + xAxis - 3 * (block - roomlength) / 4, start.y + yAxis, start.z },{ start.x + xAxis - 3 * (block - roomlength) / 4, start.y + yAxis, start.z - zAxis } ,{ start.x, start.y + yAxis, start.z - zAxis } }, { xAxis,zAxis }, out_texture, in_texture, 1, 0);
				glColor4f(1, 1, 1, 1);
				glDisable(GL_BLEND);
			...
			else
			{
				...
				glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
				glColor4f(1, 1, 1, 0.6);
				draw_quad({ { start.x, start.y, start.z },{ start.x + xAxis, start.y, start.z },{ start.x + xAxis, start.y, start.z - zAxis + 3 * (block - roomlength) / 4 } ,{ start.x, start.y, start.z - zAxis + 3 * (block - roomlength) / 4 } }, { xAxis,zAxis }, out_texture, out_texture, 1, 0);
				glColor4f(1, 1, 1, 1);
				glDisable(GL_BLEND);
				// up
				glEnable(GL_BLEND);
				glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
				//	 glColor4f(0, 0, 0, 1);
				draw_quad({ { start.x, start.y + yAxis, start.z },{ start.x + xAxis, start.y + yAxis, start.z },{ start.x + xAxis, start.y + yAxis, start.z - zAxis + 3 * (block - roomlength) / 4 } ,{ start.x, start.y + yAxis, start.z - zAxis + 3 * (block - roomlength) / 4 } }, { xAxis,zAxis }, out_texture, in_texture, 1, 0);
				glColor4f(1, 1, 1, 1);
				glDisable(GL_BLEND);
			}
		...
		if (storey_index == 0) {
			// front
			c = draw_quad({ { start.x, start.y, start.z },{ start.x + xAxis / 3, start.y, start.z },{ start.x + xAxis / 3, start.y + yAxis, start.z },{ start.x, start.y + yAxis, start.z } }, { xAxis / 3,yAxis }, out_texture, in_texture, 1, 0);
			if (!i)cu.push_back(c);
			c = draw_quad({ { start.x + 2 * xAxis / 3, start.y, start.z },{ start.x + xAxis, start.y, start.z },{ start.x + xAxis, start.y + yAxis, start.z },{ start.x + 2 * xAxis / 3, start.y + yAxis, start.z } }, { xAxis / 3,yAxis }, out_texture, in_texture, 1, 0);
			if (!i)cu.push_back(c);
			c = draw_quad({ { start.x, start.y + yAxis / 2, start.z },{ start.x + xAxis, start.y + yAxis / 2, start.z },{ start.x + xAxis, start.y + yAxis, start.z } ,{ start.x, start.y + yAxis, start.z } }, { xAxis,yAxis / 2 }, out_texture, in_texture, 1, 0);
			if (!i)cu.push_back(c);

			// back

			c = draw_quad({ { start.x, start.y, start.z - zAxis },{ start.x + xAxis, start.y, start.z - zAxis },{ start.x + xAxis, start.y + yAxis, start.z - zAxis },{ start.x, start.y + yAxis, start.z - zAxis } }, { xAxis,zAxis }, in_texture, out_texture, 1, 0);
			if (!i)cu.push_back(c);


			// left

			c = draw_quad({ { start.x, start.y, start.z - zAxis },{ start.x , start.y, start.z },{ start.x , start.y + yAxis, start.z },{ start.x, start.y + yAxis, start.z - zAxis } }, { xAxis,zAxis }, out_texture, in_texture, 1, 0);
			if (!i)cu.push_back(c);

			// right
			c = draw_quad({ { start.x + xAxis, start.y, start.z - zAxis },{ start.x + xAxis, start.y, start.z },{ start.x + xAxis, start.y + yAxis, start.z },{ start.x + xAxis, start.y + yAxis, start.z - zAxis } }, { xAxis,zAxis }, in_texture, out_texture, 1, 0);
			if (!i)cu.push_back(c);
		...
		if (!i) {
			...
			if (xAxis > zAxis) {
				...
				if (storey_index % 2 == 0)
					s = Stairs(lift_and_stairs_area.p.x, lift_and_stairs_area.p.y - 1, lift_and_stairs_area.p.z - 1, lift_and_stairs_area.xAxis / 2, lift_and_stairs_area.zAxis / 2, lift_and_stairs_area.yAxis + 2.3, 0);
				else
					s = Stairs(lift_and_stairs_area.p.x + lift_and_stairs_area.xAxis, lift_and_stairs_area.p.y - 1, lift_and_stairs_area.p.z - lift_and_stairs_area.zAxis / 2 - 1, lift_and_stairs_area.xAxis / 2, lift_and_stairs_area.zAxis / 2, lift_and_stairs_area.yAxis + 2.3, 180);
